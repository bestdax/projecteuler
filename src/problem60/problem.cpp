...
std::unordered_set<std::vector<unsigned long>, vector_hash, vector_equal> Solution::find_all_prime_pairs_set()
{
	std::unordered_set<std::vector<unsigned long>, vector_hash, vector_equal> pairs;
...
bool Solution::is_prime_set(std::unordered_set<std::vector<unsigned long>, vector_hash, vector_equal>& sets, std::vector<unsigned long> set,
                            unsigned long n)
{
	if(n <= set.back()) return false;
...
std::unordered_set<std::vector<unsigned long>, vector_hash, vector_equal> Solution::find_all_sets_with_n_members_set(unsigned n)
{
	auto sets = find_all_prime_pairs_set();

	while(n > 2)
	{
		std::unordered_set<std::vector<unsigned long>, vector_hash, vector_equal> level_sets;
...
{
	auto result = find_all_sets_with_n_members_set(5);
	unsigned long min_sum{std::numeric_limits<unsigned long>::max()};
